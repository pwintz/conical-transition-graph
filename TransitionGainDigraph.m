...
%   end % End static methods block
  ...
  methods % Define class methods.
    ...
    function this = addEdgeFromVertexToVertex(this, start_cone_ndx, end_cone_ndx, min_gain, max_gain)
      start_node_ndx = this.node_ndxs_of_vertices(start_cone_ndx);
      end_node_ndx   = this.node_ndxs_of_vertices(end_cone_ndx);
      this.addEdgeBetweenNodes(start_node_ndx, end_node_ndx, min_gain, max_gain);
    end % End of function
...
%     end % End of function
  ...
  methods(Access = {?matlab.unittest.TestCase}) % Private block, accessible by unit tests.
    ...
    function node_ndx = nodeIndexOfCone(this, cone_ndx)
      ...
      assert(cone_ndx >= 0, "Invalid cone index '%s'. Must be in {1, 2, ..., %d}", mat2str(cone_ndx), this.numVertexNodes);
      assert(cone_ndx <= this.numConeNodes, "Invalid cone index '%s'. Must be in {1, 2, ..., %d}", mat2str(cone_ndx), this.numConeNodes);
