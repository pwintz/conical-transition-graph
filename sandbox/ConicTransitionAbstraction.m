classdef ConicTransitionAbstraction < handle
  
  properties(SetAccess = immutable)
    % Define instance constants.
    conical_partition (:, 1) ConicalPartition; % Partition of state space.

    is_start_cone        (1, :) logical;
    is_end_cone          (1, :) logical;

    transition_graph            TransitionGainDigraph;
  end
...
%   end % End static methods block
  ...
  methods(Abstract) % Define class methods.
    successor_cone_ndxs = computeSuccessors(this, cone_ndx);
    out = possibleSuccessor(this, arg1, arg2);
  ...
  methods
    ...
    % │ ╰─────────────────────────────────────╯ │
    % ╰─────────────────────────────────────────╯
    function this = ConicTransitionAbstraction(conical_partitions, flow_set_cone_ndxs, jump_set_cone_ndxs, jump_set_image_cone_ndxs, flow_map_matrix, jump_map_matrix)
